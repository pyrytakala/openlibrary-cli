...
authCommand
  ...
  .addHelpText("after", "\nExample:\n  openlibrary-cli auth set sk-abc123xyz")
  .action((token: string) => {
    setToken(token);
...
authCommand
  ...
  .addHelpText("after", "\nExample:\n  openlibrary-cli auth show\n  openlibrary-cli auth show --raw")
  .action((opts: { raw?: boolean }) => {
    if (!hasToken()) {
      log.warn("No token configured. Run: openlibrary-cli auth set <token>");
      return;
    }
